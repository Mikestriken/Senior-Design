...
# Source directories
user_dir="services/root/"
root_dir="services/user/"

# Destination directory
user_destination_dir="services/rootDest/"
root_destination_dir="services/userDest/"
...
for service_file in "${user_services[@]}"; do
    if ! echo "I would enable: $(basename "$service_file" .service)"; then
...
for service_file in "${user_services[@]}"; do
    if ! echo "I would start: $(basename "$service_file" .service)"; then
        echo "Error: Enabling $(basename "$service_file" .service) service failed."
        exit 1
    fi
done
...
echo -e "\ncleaning files in 10 sec"

# sleep 10

sudo rm -rf "$user_destination_dir"
sudo rm -rf "$root_destination_dir"